...
CREATE TABLE template (
  ...
  template_type VARCHAR(50) NOT NULL CHECK (template_type IN ('form', 'output')),
...
CREATE TABLE audit_event (
  ...
  actor_type VARCHAR(20) NOT NULL CHECK (actor_type IN ('user', 'client_user', 'system')),
  actor_id UUID,
...
CREATE INDEX idx_quote_node_revision ON quote_node(revision_id);
CREATE INDEX idx_quote_node_parent ON quote_node(parent_node_id);
CREATE INDEX idx_quote_item_node ON quote_item(node_id);
...
CREATE INDEX idx_cost_event_type ON cost_event(event_type);
...
CREATE INDEX idx_audit_event_workspace ON audit_event(workspace_id);
